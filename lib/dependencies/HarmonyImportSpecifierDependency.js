...
class HarmonyImportSpecifierDependency extends HarmonyImportDependency {
	...
	getReference(moduleGraph) {
		...
		return new DependencyReference(
			this._module,
			this.getId(moduleGraph) && !this.namespaceObjectAsContext ? [this.getId(moduleGraph)] : true,
			false,
			this.sourceOrder
		);
	}
	...
	_getErrors(moduleGraph) {
		...
		const idIsNotNameMessage =
			this.getId(moduleGraph) !== this.name ? ` (imported as '${this.name}')` : "";
	...
	updateHash(hash, moduleGraph) {
		...
		hash.update(
			(importedModule && this.getId(moduleGraph) && importedModule.isUsed(this.getId(moduleGraph))) + ""
		);
		hash.update(
			(importedModule &&
