...
class ContextModule extends Module {
	...
	getFakeMap(dependencies, moduleGraph) {
		...
		const fakeMap = dependencies
			...
			.reduce((map, dep) => {
				const exportsType =
					moduleGraph.getModule(dep).buildMeta && moduleGraph.getModule(dep).buildMeta.exportsType;
				const id = moduleGraph.getModule(dep).id;
				if (!exportsType) {
	...
	getLazySource(blocks, id) {
		...
		const map = blocks
			...
			.reduce((map, item) => {
				...
				const arrayStart = [item.moduleGraph.getModule(dependency).id];
				if (typeof fakeMap === "object") {
					arrayStart.push(fakeMap[item.moduleGraph.getModule(dependency).id]);
				}
